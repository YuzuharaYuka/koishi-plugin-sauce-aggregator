// --- START OF FILE searchers/yande.ts ---
...
export class YandeReEnhancer implements Enhancer<YandeReConfig.Config> {
  ...
  constructor(public ctx: Context, public config: YandeReConfig.Config, public debug: boolean) {}
  ...
  public async enhance(result: Searcher.Result): Promise<EnhancedResult | null> {
    ...
    if (this.debug) logger.info(`[yande.re] 检测到 Yande.re 链接，帖子 ID: ${postId}，开始获取图源信息...`)

    try {
      const apiUrl = `https://yande.re/post.json?tags=id:${postId}`
      const response = await this.ctx.http.get<YandeRePost[]>(apiUrl)

      if (!response || response.length === 0) {
        if (this.debug) logger.warn(`[yande.re] API 未能找到 ID 为 ${postId} 的帖子。`)
        return null
      }
      ...
      if (postRatingLevel > maxAllowedLevel) {
        if (this.debug) logger.info(`[yande.re] 帖子 ${postId} 的评级为 '${post.rating.toUpperCase()}'，超出了配置允许的最高等级 '${this.config.maxRating.toUpperCase()}'，已跳过。`);
      ...
      if (this.debug) logger.info(`[yande.re] 正在下载图源图片 (${this.config.postQuality} 质量)... URL: ${downloadUrl}`)
