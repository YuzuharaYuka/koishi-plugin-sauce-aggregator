...
var puppeteerConfig = import_koishi.Schema.object({
  persistentBrowser: import_koishi.Schema.boolean().default(true).description("**常驻浏览器实例**<br>开启后，浏览器将在插件启动时预加载并常驻后台，响应速度最快，但会持续占用资源。关闭后，浏览器将按需启动，并在空闲后自动关闭以节省资源。"),
  browserCloseTimeout: import_koishi.Schema.number().default(30).min(0).description("**自动关闭延迟 (秒)**<br>仅在 **关闭** `常驻浏览器实例` 时生效。设置最后一次搜索任务结束后，等待多少秒关闭浏览器实例。"),
  browserLaunchTimeout: import_koishi.Schema.number().default(90).min(10).description("**浏览器启动超时 (秒)**<br>等待浏览器进程启动并准备就绪的最长时间。"),
  chromeExecutablePath: import_koishi.Schema.string().description(
    "**本地浏览器可执行文件路径 (可选)**<br>插件会优先使用此路径。如果留空，将尝试自动检测。"
  )
});
var Config = import_koishi.Schema.object({
  ...
  enhancerOrder: import_koishi.Schema.array(import_koishi.Schema.object({
    engine: import_koishi.Schema.union(["gelbooru", "yandere", "danbooru", "pixiv"]).description("图源"),
    enabled: import_koishi.Schema.boolean().default(true).description("是否启用")
  })).role("table").default([
  ...
  pixiv: import_koishi.Schema.object({
    refreshToken: import_koishi.Schema.string().role("secret").description("Pixiv API Refresh Token. 用于 API 请求认证，是启用此图源的必需项。\n\n获取教程: https://www.nanoka.top/posts/e78ef86/"),
    postQuality: import_koishi.Schema.union([
      import_koishi.Schema.const("original").description("原图"),
      import_koishi.Schema.const("large").description("大图"),
      import_koishi.Schema.const("medium").description("中等图")
    ]).default("large").description("图片质量。从`Pixiv`获取的图片尺寸。"),
    allowR18: import_koishi.Schema.boolean().default(false).description("是否允许发送 R-18/R-18G 内容。"),
  ...
  debug: import_koishi.Schema.object({
    ...
    logApiResponses: import_koishi.Schema.array(import_koishi.Schema.union(["saucenao", "iqdb", "tracemoe", "yandex", "ascii2d", "soutubot", "gelbooru", "yandere", "danbooru", "pixiv"])).role("checkbox").default([]).description("记录 API 响应。选择要将 API 或页面原始返回信息输出到日志的引擎/图源 (可能产生大量日志)。")
    ...
    try {
      ...
      return uniqueResults.slice(0, options.maxResults).map((res) => {
        ...
        const formatTime = /* @__PURE__ */ __name((seconds) => {
          const h10 = Math.floor(seconds / 3600).toString().padStart(2, "0");
...
function getImageUrlAndName(session, text) {
  ...
  if (session.app?.config.debug) {
    logger3.info(`[Debug] Parsed image URL: ${url}`);
  }
...
var DanbooruEnhancer = class {
  ...
  async enhance(result) {
    ...
    try {
      ...
      if (this.debugConfig.enabled) logger10.info(`[danbooru] [Stealth] 正在通过页面内 fetch 获取 API: ${apiBaseUrl}`);
      ...
      if (downloadUrl) {
        if (this.debugConfig.enabled) logger10.info(`[danbooru] [Stealth] 正在通过页面内 fetch 下载图源图片: ${downloadUrl}`);
        ...
        if (this.debugConfig.enabled) logger10.info(`[danbooru] [Stealth] 图片下载并转换成功，大小: ${imageBuffer.length} 字节。`);
      }
...
// src/enhancers/pixiv.ts
var import_koishi12 = require("koishi");
var logger11 = new import_koishi12.Logger("sauce-aggregator");
var PixivApiService = class {
  constructor(ctx, config, debugConfig, requestTimeout) {
    this.ctx = ctx;
    this.config = config;
    this.debugConfig = debugConfig;
  ...
  async _refreshAccessToken() {
    ...
    try {
      ...
      if (response.access_token) {
        ...
        if (this.debugConfig.enabled) logger11.info("[pixiv] AccessToken 刷新成功。");
        return true;
      }
      return false;
    } catch (error) {
      this.accessToken = null;
      logger11.error("[pixiv] 刷新 AccessToken 失败:", error.response?.data || error.message);
      return false;
    }
  }
  async _request(url, params) {
    ...
    const makeRequest = /* @__PURE__ */ __name(() => this.ctx.http.get(url, {
      params,
    ...
    try {
      ...
      if (error.response?.status === 400 && /invalid_grant|invalid_token/i.test(errorMsg)) {
        if (this.debugConfig.enabled) logger11.info("[pixiv] AccessToken 已失效，尝试强制刷新...");
        if (await this._refreshAccessToken()) {
          return await makeRequest();
        }
      }
  ...
  async getArtworkDetail(pid) {
    try {
      ...
      if (this.debugConfig.enabled) logger11.warn(`[pixiv] 获取插画详情失败 (PID: ${pid}):`, error.response?.data || error.message);
      return null;
    }
  }
  async downloadImage(url) {
    try {
      ...
      logger11.warn(`[pixiv] 图片下载失败 (URL: ${url}):`, error.message);
      return null;
    }
  }
};
var PixivEnhancer = class {
  constructor(ctx, config, debugConfig, requestTimeout) {
    this.ctx = ctx;
    this.config = config;
    this.debugConfig = debugConfig;
    this.api = new PixivApiService(ctx, config, debugConfig, requestTimeout * 1e3);
  }
  static {
    __name(this, "PixivEnhancer");
  }
  name = "pixiv";
  api;
  async enhance(result) {
    ...
    if (!postId) return null;
    if (this.debugConfig.enabled) logger11.info(`[pixiv] 检测到 Pixiv 链接 (来自 ${pixivUrl})，帖子 ID: ${postId}，开始获取图源信息...`);
    try {
      const illust = await this.api.getArtworkDetail(postId);
      if (!illust) {
        if (this.debugConfig.enabled) logger11.warn(`[pixiv] API 未能找到 ID 为 ${postId} 的帖子。`);
        return null;
      }
      if (this.debugConfig.logApiResponses.includes(this.name)) {
        logger11.info(`[pixiv] API 响应: ${JSON.stringify(illust, null, 2)}`);
      }
      const isR18 = illust.x_restrict > 0;
      if (isR18 && !this.config.allowR18) {
        if (this.debugConfig.enabled) logger11.info(`[pixiv] 帖子 ${postId} 是 R-18 内容，但配置不允许发送，已跳过。`);
        return { details: [import_koishi12.h.text(`[!] Pixiv 图源的评级为 R-18，根据设置已隐藏详情。`)] };
      }
      ...
      if (illust.meta_pages && illust.meta_pages.length > 0) {
        ...
        switch (this.config.postQuality) {
          case "original":
            downloadUrl = image_urls.original;
            break;
          case "large":
            downloadUrl = image_urls.large;
            break;
          case "medium":
            downloadUrl = image_urls.medium;
            break;
          default:
            downloadUrl = image_urls.large;
            break;
      ...
      if (!downloadUrl) {
        if (this.debugConfig.enabled) logger11.warn(`[pixiv] 帖子 ${postId} 缺少任何可用的图片 URL。`);
        return { details };
      }
      if (this.debugConfig.enabled) logger11.info(`[pixiv] 正在下载图源图片 (${this.config.postQuality} 质量)... URL: ${downloadUrl}`);
      const imageBuffer = await this.api.downloadImage(downloadUrl);
      const imageType = getImageTypeFromUrl(downloadUrl);
      return { details, imageBuffer, imageType };
    } catch (error) {
      logger11.error(`[pixiv] 获取图源信息 (ID: ${postId}) 时发生错误:`, error);
      return null;
    }
  }
  findPixivUrl(result) {
  ...
  buildDetailNodes(illust) {
    ...
    info.push(tagsText);
    return [import_koishi12.h.text(info.join("\n"))];
  }
};
...
var logger12 = new import_koishi13.Logger("sauce-aggregator:puppeteer");
...
var PuppeteerManager = class {
  ...
  _closeTimer = null;
  // --- THIS IS THE FIX ---: Timer for auto-closing
  ...
  async initialize() {
    ...
    logger12.info("[Stealth] 正在预初始化常驻浏览器实例...");
    try {
      await this.getBrowser();
      this._isInitialized = true;
      logger12.info("[Stealth] 常驻浏览器实例已成功预初始化。");
    } catch (error) {
      logger12.error("[Stealth] 预初始化常驻浏览器实例失败:", error);
    }
  }
  async getBrowserPath() {
    const customPath = this.config.puppeteer.chromeExecutablePath;
    if (customPath) {
      if (this.config.debug.enabled) logger12.info(`[Stealth] 使用用户配置的浏览器路径: ${customPath}`);
      return customPath;
    }
    try {
      if (this.config.debug.enabled) logger12.info("[Stealth] 正在使用 puppeteer-finder 自动检测浏览器...");
      const browserPath = await (0, import_puppeteer_finder.default)();
      logger12.info(`[Stealth] 自动检测到浏览器路径: ${browserPath}`);
      return browserPath;
    } catch (error) {
      logger12.warn("[Stealth] puppeteer-finder 未能找到任何浏览器:", error);
      return null;
    }
  }
  async launchBrowser() {
    ...
    const launchTimeout = this.config.puppeteer.browserLaunchTimeout * 1e3;
    const browser = await import_puppeteer_extra.default.launch({
      ...
      timeout: this.config.requestTimeout * 1e3
    });
    browser.on("disconnected", () => {
      logger12.warn("[Stealth] 共享浏览器实例已断开连接。");
  ...
  getBrowser() {
    if (this._closeTimer) {
      ...
      if (this.config.debug.enabled) logger12.info("[Stealth] [按需模式] 浏览器被再次使用，已取消自动关闭。");
    }
    if (this._browserPromise) {
      return this._browserPromise.then((browser) => {
        ...
        if (this.config.debug.enabled) logger12.info("[Stealth] 共享浏览器实例已断开，正在启动新的实例...");
    ...
    if (this.config.debug.enabled) logger12.info("[Stealth] 共享浏览器实例不存在，正在启动...");
    this._browserPromise = this.launchBrowser().catch((err) => {
      this._browserPromise = null;
      throw err;
    });
  ...
  async scheduleClose() {
    ...
    if ((await browser.pages()).length > 1) {
      if (this.config.debug.enabled) logger12.info(`[Stealth] [按需模式] 仍有 ${(await browser.pages()).length - 1} 个活动页面，暂不关闭。`);
      return;
    }
    const timeout = this.config.puppeteer.browserCloseTimeout * 1e3;
    if (timeout <= 0) {
      if (this.config.debug.enabled) logger12.info("[Stealth] [按需模式] 关闭延迟为0，立即关闭浏览器。");
      this.dispose();
      return;
    }
    if (this.config.debug.enabled) logger12.info(`[Stealth] [按需模式] 所有页面已关闭，将在 ${timeout / 1e3} 秒后自动关闭浏览器。`);
    this._closeTimer = setTimeout(() => {
      if (this.config.debug.enabled) logger12.info("[Stealth] [按需模式] 空闲超时，正在关闭浏览器实例...");
      this.dispose();
      this._closeTimer = null;
    }, timeout);
  }
  ...
  async saveErrorSnapshot(page, contextName) {
    try {
      ...
      logger12.info(`[Stealth] [${contextName}] 已保存错误快照: ${screenshotPath}`);
      logger12.info(`[Stealth] [${contextName}] 已保存错误页面HTML: ${htmlPath}`);
    } catch (snapshotError) {
      logger12.error(`[Stealth] [${contextName}] 保存错误快照失败:`, snapshotError);
    }
  }
  async dispose() {
    if (this._closeTimer) {
    ...
    if (this._browserPromise) {
      try {
        ...
        if (browser?.isConnected()) {
          if (this.config.debug.enabled) logger12.info("[Stealth] 正在关闭共享浏览器实例...");
          await browser.close();
        }
      } catch (error) {
        logger12.warn("[Stealth] 关闭浏览器实例时发生错误:", error);
      }
      this._browserPromise = null;
    }
  }
...
var logger13 = new import_koishi14.Logger("sauce-aggregator:message-builder");
async function createResultContent(ctx, result, engineName) {
  ...
  const textNode = import_koishi14.h.text("\n" + textFields.join("\n"));
  try {
    const imageBuffer = Buffer.from(await ctx.http.get(result.thumbnail, { responseType: "arraybuffer" }));
    const imageBase64 = imageBuffer.toString("base64");
    const dataUri = `data:image/jpeg;base64,${imageBase64}`;
    return [import_koishi14.h.image(dataUri), textNode];
  } catch (e) {
    logger13.warn(`缩略图下载失败 ${result.thumbnail}:`, e.message);
    return [(0, import_koishi14.h)("p", "[!] 缩略图加载失败"), textNode];
  }
}
...
async function buildLowConfidenceNode(ctx, result, engineName, botUser) {
  ...
  return (0, import_koishi14.h)("message", {
...
async function buildHighConfidenceMessage(figureMessage, ctx, config, sortedEnhancers, result, engineName, botUser) {
  if (result.coverImage) {
    figureMessage.children.push((0, import_koishi14.h)("message", { nickname: "番剧封面", avatar: botUser.avatar }, import_koishi14.h.image(result.coverImage)));
  }
  const formattedContent = await createResultContent(ctx, result);
  const detailsNode = (0, import_koishi14.h)("message", { nickname: "详细信息", avatar: botUser.avatar }, formattedContent);
  figureMessage.children.push(detailsNode);
  if (engineName === "tracemoe" && config.tracemoe.sendVideoPreview && result.url) {
    try {
      if (config.debug.enabled) logger13.info(`[tracemoe] 正在为高置信度结果下载视频预览...`);
      const videoPreview = await ctx.http.get(result.url, { responseType: "arraybuffer" });
      figureMessage.children.push((0, import_koishi14.h)("message", { nickname: "视频预览", avatar: botUser.avatar }, import_koishi14.h.video(videoPreview, "video/mp4")));
    } catch (e) {
      logger13.warn(`[tracemoe] 高置信度视频预览下载失败: ${e.message}`);
    }
  }
  for (const enhancer of sortedEnhancers) {
    try {
      ...
      if (enhancedData) {
        if (config.debug.enabled) logger13.info(`[${enhancer.name}] 已成功获取图源信息。`);
        if (enhancedData.imageBuffer) {
          figureMessage.children.push((0, import_koishi14.h)("message", { nickname: "图源图片", avatar: botUser.avatar }, import_koishi14.h.image(enhancedData.imageBuffer, enhancedData.imageType)));
        }
        const enhancedDetailsNode = (0, import_koishi14.h)("message", { nickname: "图源信息", avatar: botUser.avatar }, enhancedData.details);
        figureMessage.children.push(enhancedDetailsNode);
        break;
      }
    } catch (e) {
      logger13.warn(`[${enhancer.name}] 图源处理时发生错误:`, e);
    }
  }
}
...
async function sendFigureMessage(session, figureMessage, errorMessage) {
  if (figureMessage.children.length > 0) {
    try {
      ...
      logger13.warn(`${errorMessage}:`, e.message);
...
// src/core/search-handler.ts
var logger14 = new import_koishi15.Logger("sauce-aggregator:handler");
var SearchHandler = class {
  ...
  async performSearch(searcher, options) {
    try {
      ...
      logger14.warn(errorMessage, this.config.debug.enabled ? error : "");
  ...
  async handleDirectSearch(searchers, options, botUser, session, collectedErrors) {
    ...
    const figureMessage = (0, import_koishi15.h)("figure");
  ...
  async handleSequentialSearch(searchers, options, botUser, session, collectedErrors, sortedEnhancers) {
    ...
    if (highConfidenceResults.length > 0) {
      ...
      const figureMessage2 = (0, import_koishi15.h)("figure");
      ...
      if (eng.config.alwaysAttach && eng.searcher) {
        ...
        if (output?.results?.[0]) {
          if (figureMessage) {
            ...
            const headerNode = (0, import_koishi15.h)("message", { nickname: `--- ${eng.name} (附加结果) ---`, avatar: botUser.avatar });
...
var using = ["http"];
var inject = ["http"];
var logger15 = new import_koishi16.Logger(name);
var usage = `
###	指令用法
#### sauce [引擎名] [图片]
*	别名: \`搜图\`, \`soutu\`
...
function apply(ctx, config) {
  ...
  ctx.on("ready", async () => {
    if (config.puppeteer.persistentBrowser) {
      const puppeteerSearchers = ["yandex", "ascii2d", "soutubot"];
  ...
  if (config.saucenao.apiKeys && config.saucenao.apiKeys.length > 0) {
    ...
    logger15.info("[saucenao] 未提供任何 API Key，引擎已禁用。");
  }
  ...
  for (const name2 in enhancerRegistry) {
    ...
    const areKeysProvided = entry.needsKeys ? Array.isArray(entry.keys) ? entry.keys.filter(Boolean).length > 0 : !!entry.keys : true;
    if (areKeysProvided) {
      const constructorArgs = [ctx, generalConfig, config.debug];
      if (name2 === "yandere" || name2 === "gelbooru" || name2 === "pixiv") constructorArgs.push(config.requestTimeout);
      if (entry.requiresPuppeteer) constructorArgs.push(puppeteerManager);
      allEnhancers[name2] = new entry.constructor(...constructorArgs);
    } else {
      logger15.info(`[${name2}] ${entry.messageName}未配置任何${entry.keyName}，将无法启用。`);
    }
  }
  ...
  if (allEnabledSearchers.length > 0) {
    logger15.info(`已启用的搜图引擎顺序: ${allEnabledSearchers.map((s) => s.name).join(", ")}`);
  }
  if (sortedEnhancers.length > 0) {
    logger15.info(`已启用的图源顺序: ${sortedEnhancers.map((e) => e.name).join(", ")}`);
  }
  ...
  ctx.command("sauce [...text:string]", "聚合搜图").alias("soutu", "搜图").option("all", "-a, --all 返回所有启用的引擎搜索结果").action(async ({ session, options }, text) => {
    ...
    if (!imgData.url) {
      ...
      try {
        ...
        const unescapedContent = import_koishi16.h.unescape(nextMessageContent);
        const messageSession = { content: unescapedContent, elements: import_koishi16.h.parse(unescapedContent) };
        imgData = getImageUrlAndName(messageSession, unescapedContent);
        if (!imgData.url) return "未找到图片，已取消。";
      } catch (e) {
        if (config.debug.enabled) logger15.warn("等待用户图片时出错:", e);
        return "等待超时，已取消。";
      }
    }
    try {
      ...
      if (!/\.(jpe?g|png|gif|webp)$/i.test(imgData.name)) {
        ...
        if (imageType) {
          ...
          if (config.debug.enabled) logger15.info(`[Debug] Original filename "${imgData.name}" lacked extension. Renaming to "${newName}".`);
