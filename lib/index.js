...
var Config = import_koishi.Schema.object({
  order: import_koishi.Schema.array(import_koishi.Schema.object({
    engine: import_koishi.Schema.union(["saucenao", "iqdb", "tracemoe", "yandex", "ascii2d"]).description("搜图引擎"),
  ...
  confidenceThreshold: import_koishi.Schema.number().default(85).min(0).max(100).description("高匹配度阈值 (%)。结果相似度高于此值时将直接返回，不再继续搜索。"),
  ...
  debug: import_koishi.Schema.object({
    ...
    logApiResponses: import_koishi.Schema.array(import_koishi.Schema.union(["saucenao", "iqdb", "tracemoe", "yandex", "ascii2d", "gelbooru", "yandere", "danbooru"])).role("checkbox").default([]).description("记录 API 响应。选择要将 API 或页面原始返回信息输出到日志的引擎/图源 (可能产生大量日志)。")
...
var SauceNAO = class {
  constructor(ctx, config, debugConfig, pluginConfig) {
    this.ctx = ctx;
    this.config = config;
    this.debugConfig = debugConfig;
    this.timeout = pluginConfig.requestTimeout * 1e3;
  }
  static {
    ...
    try {
      ...
      return uniqueResults.slice(0, options.maxResults).map((res) => {
        ...
        const formatTime = /* @__PURE__ */ __name((seconds) => {
          const h6 = Math.floor(seconds / 3600).toString().padStart(2, "0");
...
var IQDB = class {
  ...
  async search(options) {
    ...
    if (this.debugConfig.enabled) logger3.info(`[iqdb] 发送请求到 ${url}，图片大小: ${options.imageBuffer.length} 字节`);
    try {
      ...
      if (this.debugConfig.enabled) logger3.info(`[iqdb] 收到响应页面，长度: ${html.length}`);
      if (this.debugConfig.logApiResponses.includes(this.name)) {
        logger3.info(`[iqdb] Raw HTML: ${html.substring(0, 2e3)}...`);
      }
      ...
      if (resultElements.length === 0) {
        ...
        if (this.debugConfig.enabled) {
          logger3.warn("[iqdb] 页面结构可能已更改，未找到结果容器。");
          logger3.info(`[iqdb] Raw HTML for debugging:
${html}`);
        }
        return [];
      }
      resultElements.each((_, element) => {
        try {
          ...
          if (this.debugConfig.enabled) logger3.error("[iqdb] 解析单个结果时出错:", parseError);
        }
      });
...
var Yandex = class {
  ...
  async search(options) {
    ...
    try {
      ...
      if (this.debugConfig.enabled) logger4.info(`[yandex] [Stealth] 导航到: ${url}`);
      await page.goto(url);
      ...
      if (this.debugConfig.enabled) logger4.info(`[yandex] [Stealth] 正在上传临时文件: ${tempFilePath}`);
      await inputUploadHandle.uploadFile(tempFilePath);
      if (this.debugConfig.enabled) logger4.info(`[yandex] [Stealth] 等待页面跳转...`);
      await page.waitForNavigation({ waitUntil: "networkidle0" });
      if (this.debugConfig.enabled) logger4.info(`[yandex] [Stealth] 正在解析结果页面: ${page.url()}`);
      const html = await page.content();
      if (this.debugConfig.logApiResponses.includes(this.name)) {
        logger4.info(`[yandex] Raw HTML length: ${html.length}.`);
      }
      const $ = cheerio2.load(html);
      const dataStateAttr = $('div.Root[id^="ImagesApp-"]').attr("data-state");
      if (!dataStateAttr) {
        if (this.debugConfig.enabled) logger4.warn("[yandex] 页面结构可能已更改，未找到 data-state 属性。");
        return [];
      }
      ...
      if (this.debugConfig.enabled) logger4.info(`[yandex] 成功解析到 ${results.length} 个结果。`);
      return results;
    } catch (error) {
      logger4.warn(`[yandex] 请求或解析出错: ${error.message}`);
      if (this.debugConfig.enabled) logger4.debug(`[yandex] 错误详情:`, error);
...
var Ascii2D = class {
  ...
  async search(options) {
    if (!options.imageUrl) {
      logger5.warn("[ascii2d] 此引擎需要图片 URL 才能进行搜索。");
      return [];
    }
    const page = await this.puppeteer.getPage();
    try {
      if (this.debugConfig.enabled) logger5.info(`[ascii2d] [Stealth] 导航到 ascii2d.net`);
      ...
      if (this.debugConfig.enabled) logger5.info(`[ascii2d] [Stealth] 正在快速输入 URL...`);
      ...
      if (this.debugConfig.enabled) logger5.info(`[ascii2d] [Stealth] 点击 URL 搜索按钮...`);
      await Promise.all([
      ...
      if (this.debugConfig.enabled) logger5.info(`[ascii2d] [Stealth] 已加载颜色搜索 (color) 结果页: ${page.url()}`);
      if (this.debugConfig.enabled) logger5.info(`[ascii2d] [Stealth] 正在解析最终结果页面...`);
      const results = await this.parseResults(page);
      return results.slice(0, options.maxResults);
    } catch (error) {
      logger5.error("[ascii2d] [Stealth] 搜索过程中发生错误:", error);
      if (this.debugConfig.enabled) {
        try {
          const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
          const logDir = import_path2.default.resolve(this.ctx.baseDir, "logs");
          await import_fs2.promises.mkdir(logDir, { recursive: true });
          const screenshotPath = import_path2.default.resolve(logDir, `ascii2d-error-${timestamp}.png`);
          const htmlPath = import_path2.default.resolve(logDir, `ascii2d-error-${timestamp}.html`);
          await page.screenshot({ path: screenshotPath, fullPage: true });
          const htmlContent = await page.content();
          await import_fs2.promises.writeFile(htmlPath, htmlContent);
          logger5.info(`[ascii2d] [Stealth] 已保存错误快照: ${screenshotPath}`);
          logger5.info(`[ascii2d] [Stealth] 已保存错误页面HTML: ${htmlPath}`);
        } catch (snapshotError) {
          logger5.error("[ascii2d] [Stealth] 保存错误快照失败:", snapshotError);
        }
      }
      throw error;
    } finally {
...
var YandeReEnhancer = class {
  ...
  async enhance(result) {
    ...
    if (this.debugConfig.enabled) logger6.info(`[yande.re] 检测到 Yande.re 链接，帖子 ID: ${postId}，开始获取图源信息...`);
    try {
      const apiUrl = `https://yande.re/post.json?tags=id:${postId}`;
      const response = await this.ctx.http.get(apiUrl, { timeout: this.timeout });
      if (this.debugConfig.logApiResponses.includes(this.name)) {
        logger6.info(`[yande.re] API 响应: ${JSON.stringify(response, null, 2)}`);
      }
      if (!response || response.length === 0) {
        if (this.debugConfig.enabled) logger6.warn(`[yande.re] API 未能找到 ID 为 ${postId} 的帖子。`);
        return null;
      }
      ...
      if (postRatingLevel > maxAllowedLevel) {
        if (this.debugConfig.enabled) logger6.info(`[yande.re] 帖子 ${postId} 的评级为 '${post.rating.toUpperCase()}'，超出了配置允许的最高等级 '${this.config.maxRating.toUpperCase()}'，已跳过。`);
        return { details: [import_koishi7.h.text(`[!] Yande.re 图源的评级 (${post.rating.toUpperCase()}) 超出设置，已隐藏详情。`)] };
      }
      ...
      if (this.debugConfig.enabled) logger6.info(`[yande.re] 正在下载图源图片 (${this.config.postQuality} 质量)... URL: ${downloadUrl}`);
      ...
      logger6.error(`[yande.re] 获取图源信息 (ID: ${postId}) 时发生错误:`, error);
      return null;
    }
  }
  ...
  buildDetailNodes(post) {
    ...
    info.push(tagsText);
    return [import_koishi7.h.text(info.join("\n"))];
  }
};
...
var GelbooruEnhancer = class {
  ...
  async enhance(result) {
    ...
    if (!postId && !postMd5) {
      if (this.debugConfig.enabled) logger7.info(`[gelbooru] 在链接 ${gelbooruUrl} 中未找到有效的 id 或 md5 参数。`);
      return null;
    }
    const logIdentifier = postId ? `ID: ${postId}` : `MD5: ${postMd5}`;
    if (this.debugConfig.enabled) logger7.info(`[gelbooru] 检测到 Gelbooru 链接，${logIdentifier}，开始获取图源信息...`);
    try {
      ...
      if (this.debugConfig.logApiResponses.includes(this.name)) {
        logger7.info(`[gelbooru] API 响应: ${JSON.stringify(response, null, 2)}`);
      }
      const post = response?.post?.[0];
      if (!post || !post.id) {
        if (this.debugConfig.enabled) logger7.warn(`[gelbooru] API 未能找到帖子 (${logIdentifier})。`);
        return null;
      }
      ...
      const maxAllowedLevel = ratingHierarchy[this.config.maxRating];
      if (postRatingLevel > maxAllowedLevel) {
        if (this.debugConfig.enabled) logger7.info(`[gelbooru] 帖子 ${post.id} 的评级 '${post.rating}' 超出设置 '${this.config.maxRating}'，已跳过。`);
        return { details: [import_koishi8.h.text(`[!] Gelbooru 图源的评级 (${post.rating}) 超出设置，已隐藏详情。`)] };
      }
      const details = this.buildDetailNodes(post);
      let downloadUrl;
      switch (this.config.postQuality) {
      ...
      if (!downloadUrl) {
        if (this.debugConfig.enabled) logger7.warn(`[gelbooru] 帖子 ${post.id} 缺少 ${this.config.postQuality} 质量的图片URL，将尝试使用 sample_url。`);
        downloadUrl = post.sample_url;
      }
      if (!downloadUrl) {
        if (this.debugConfig.enabled) logger7.warn(`[gelbooru] 帖子 ${post.id} 缺少任何可用的图片URL。`);
        return { details };
      }
      if (this.debugConfig.enabled) logger7.info(`[gelbooru] 正在下载图源图片 (${this.config.postQuality} 质量)... URL: ${downloadUrl}`);
      const imageBuffer = Buffer.from(await this.ctx.http.get(downloadUrl, { responseType: "arraybuffer", timeout: this.timeout }));
      const imageType = this.getImageType(downloadUrl);
      return { details, imageBuffer, imageType };
    } catch (error) {
      logger7.error(`[gelbooru] 获取图源信息 (${logIdentifier}) 时发生错误:`, error);
      return null;
    }
  }
  ...
  buildDetailNodes(post) {
    ...
    info.push(tagsText);
    return [import_koishi8.h.text(info.join("\n"))];
  }
};
...
var DanbooruEnhancer = class {
  ...
  async enhance(result) {
    ...
    if (!postId) {
      if (this.debugConfig.enabled) logger8.info(`[danbooru] 在链接 ${danbooruUrl} 中未找到有效的帖子 ID。`);
      return null;
    }
    const page = await this.puppeteer.getPage();
    try {
      ...
      if (this.debugConfig.enabled) logger8.info(`[danbooru] [Stealth] 正在通过 fetch 获取 API: ${apiBaseUrl}`);
      ...
      if (this.debugConfig.logApiResponses.includes(this.name)) {
        logger8.info(`[danbooru] API 响应: ${JSON.stringify(post, null, 2)}`);
      }
      ...
      if (downloadUrl) {
        if (this.debugConfig.enabled) logger8.info(`[danbooru] [Stealth] 正在下载图源图片: ${downloadUrl}`);
        ...
        if (this.debugConfig.enabled) logger8.info(`[danbooru] [Stealth] 图片下载成功，大小: ${imageBuffer.length} 字节。`);
      }
      ...
      const maxAllowedLevel = ratingHierarchy[this.config.maxRating];
      if (postRatingLevel > maxAllowedLevel) {
        if (this.debugConfig.enabled) logger8.info(`[danbooru] 帖子 ${post.id} 的评级 '${postRating}' 超出设置 '${this.config.maxRating}'，已跳过。`);
        return { details: [import_koishi9.h.text(`[!] Danbooru 图源的评级 (${postRating}) 超出设置，已隐藏详情。`)] };
      }
      const details = this.buildDetailNodes(post);
      const imageType = this.getImageType(post.file_url);
      return { details, imageBuffer, imageType };
    } catch (error) {
      logger8.error(`[danbooru] [Stealth] 处理过程中发生错误 (ID: ${postId}):`, error);
      throw error;
    } finally {
      if (page && !page.isClosed()) await page.close();
    }
...
var logger9 = new import_koishi10.Logger("sauce-aggregator:puppeteer");
...
var PuppeteerManager = class {
  ...
  async getBrowserPath() {
    if (this.config.chromeExecutablePath) {
      if (this.config.debug.enabled) logger9.info(`[Stealth] 使用用户配置的全局浏览器路径: ${this.config.chromeExecutablePath}`);
      return this.config.chromeExecutablePath;
    }
    try {
      if (this.config.debug.enabled) logger9.info("[Stealth] 正在使用 puppeteer-finder 自动检测浏览器...");
      const browserPath = await (0, import_puppeteer_finder.default)();
      logger9.info(`[Stealth] 自动检测到浏览器路径: ${browserPath}`);
      return browserPath;
    } catch (error) {
      logger9.warn("[Stealth] puppeteer-finder 未能找到任何浏览器:", error);
      return null;
    }
  }
  async launchBrowser() {
    ...
    browser.on("disconnected", () => {
      logger9.warn("[Stealth] 共享浏览器实例已断开连接。");
  ...
  getBrowser() {
    if (this._browserPromise) {
      return this._browserPromise.then((browser) => {
        ...
        if (this.config.debug.enabled) logger9.info("[Stealth] 共享浏览器实例已断开，正在启动新的实例...");
    ...
    if (this.config.debug.enabled) logger9.info("[Stealth] 共享浏览器实例不存在，正在启动...");
    this._browserPromise = this.launchBrowser().catch((err) => {
      this._browserPromise = null;
      throw err;
    });
  ...
  async dispose() {
    if (this._browserPromise) {
      try {
        ...
        if (browser?.isConnected()) {
          if (this.config.debug.enabled) logger9.info("[Stealth] 正在关闭共享浏览器实例...");
          await browser.close();
        }
      } catch (error) {
        logger9.warn("[Stealth] 关闭浏览器实例时发生错误:", error);
      }
      this._browserPromise = null;
    }
  }
...
var using = ["http"];
var inject = ["http"];
var logger10 = new import_koishi11.Logger(name);
var usage = `
指令: sauce [引擎名] [图片]
别名: 搜图, soutu
选项: --all / -a (返回全部引擎搜索结果)
...
async function preprocessImage(buffer, maxSizeInMB = 4) {
  ...
  logger10.info(`图片体积 (${(buffer.length / ONE_MB).toFixed(2)}MB) 超出 ${maxSizeInMB}MB，正在压缩...`);
  try {
    const imageType = detectImageType(buffer);
    if (!imageType) {
      logger10.warn(`[preprocess] 不支持的图片格式，无法压缩。将使用原图。`);
      return buffer;
    }
    const stream = import_stream.Readable.from(buffer);
    const image = imageType === "jpeg" ? await PImage.decodeJPEGFromStream(stream) : await PImage.decodePNGFromStream(stream);
    ...
    const encodePromise = imageType === "jpeg" ? PImage.encodeJPEGToStream(newCanvas, passThrough, 90) : PImage.encodePNGToStream(newCanvas, passThrough);
    await encodePromise;
    const finalBuffer = import_buffer.Buffer.concat(chunks);
    logger10.info(`[preprocess] 图片压缩完成，新体积: ${(finalBuffer.length / ONE_MB).toFixed(2)}MB`);
    return finalBuffer;
  } catch (error) {
    logger10.error("图片压缩失败:", error);
    return buffer;
  }
}
...
function apply(ctx, config) {
  ...
  if (config.saucenao.apiKeys && config.saucenao.apiKeys.length > 0) {
    ...
    logger10.info("[saucenao] 未提供任何 API Key，引擎已禁用。");
  }
  ...
  for (const name2 in enhancerRegistry) {
    ...
    if (!entry.needsKeys || Array.isArray(entry.keys) && entry.keys.length > 0) {
      ...
      if (name2 === "yandere" || name2 === "gelbooru") constructorArgs.push(config);
      if (entry.requiresPuppeteer) constructorArgs.push(puppeteerManager);
      allEnhancers[name2] = new entry.constructor(...constructorArgs);
    } else {
      logger10.info(`[${name2}] ${entry.messageName}未配置任何${entry.keyName}，将无法启用。`);
    }
  }
  ...
  if (allEnabledSearchers.length > 0) {
    logger10.info(`已启用的搜图引擎顺序: ${allEnabledSearchers.map((s) => s.name).join(", ")}`);
  }
  if (sortedEnhancers.length > 0) {
    logger10.info(`已启用的图源顺序: ${sortedEnhancers.map((e) => e.name).join(", ")}`);
  }
  ...
  function parseInput(inputText, options) {
    const text = inputText || "";
    const words = text.split(/\s+/).filter(Boolean);
    let searchersToUse = sequentialSearchers;
    let imageInput = text;
  ...
  async function searchLogic(searchers, image, isSingleEngineSpecified, { session, options }) {
    ...
    if (!imgData.url) {
      ...
      try {
        ...
        imgData = getImageUrlAndName({ content: nextMessageContent, quote: session.quote, elements: import_koishi11.h.parse(nextMessageContent) }, nextMessageContent);
    ...
    try {
      ...
      logger10.error("图片处理失败:", error);
      return "图片处理失败，请检查链接或网络。";
    }
  }
  __name(searchLogic, "searchLogic");
  async function performSearch(searcher, options, session) {
    try {
      ...
      logger10.warn(errorMessage, config.debug.enabled ? error : "");
  ...
  async function sendFigureMessage(session, figureMessage, errorMessage) {
    if (figureMessage.children.length > 0) {
      try {
        ...
        logger10.warn(`${errorMessage}:`, e.message);
        await session.send("结果发送失败，请检查适配器兼容性。");
      }
    }
  }
  __name(sendFigureMessage, "sendFigureMessage");
  async function handleDirectSearch(searchers, options, botUser, session) {
    ...
    const figureMessage = (0, import_koishi11.h)("figure");
    const nodePromises = successfulOutputs.flatMap((output) => {
      const headerNode = Promise.resolve((0, import_koishi11.h)("message", { nickname: `--- ${output.engine} ---`, avatar: botUser.avatar }));
      const resultNodesPromises = output.results.slice(0, config.maxResults).map(
        (result) => buildLowConfidenceNode(result, output.engine, botUser)
      );
  ...
  __name(handleDirectSearch, "handleDirectSearch");
  async function handleSequentialSearch(searchers, options, botUser, session) {
    const executedOutputs = [];
    let highConfidenceResult = null;
    ...
    if (highConfidenceResult) {
      ...
      const figureMessage2 = (0, import_koishi11.h)("figure");
      await buildHighConfidenceMessage(figureMessage2, highConfidenceResult, highConfidenceSearcherName, botUser);
      await attachAdditionalResults(executedOutputs, options, botUser, figureMessage2, session);
      await sendFigureMessage(session, figureMessage2, "发送高匹配度结果失败");
      return;
    }
    let finalOutputs = executedOutputs;
    const searchersToRunForLowConfidence = allEnabledSearchers.filter((s) => {
  ...
  __name(handleSequentialSearch, "handleSequentialSearch");
      ...
      if (eng.config.alwaysAttach) {
        ...
        if (output?.results?.[0]) {
          if (figureMessage) {
            ...
            const headerNode = (0, import_koishi11.h)("message", { nickname: `--- ${eng.name} (附加结果) ---`, avatar: botUser.avatar });
  ...
  __name(attachAdditionalResults, "attachAdditionalResults");
  async function buildHighConfidenceMessage(figureMessage, result, engineName, botUser) {
    if (result.coverImage) {
      figureMessage.children.push((0, import_koishi11.h)("message", { nickname: "番剧封面", avatar: botUser.avatar }, import_koishi11.h.image(result.coverImage)));
    }
    const formattedContent = await createResultContent(result);
    const detailsNode = (0, import_koishi11.h)("message", { nickname: "详细信息", avatar: botUser.avatar }, formattedContent);
    figureMessage.children.push(detailsNode);
    if (engineName === "tracemoe" && config.tracemoe.sendVideoPreview && result.url) {
      try {
        if (config.debug.enabled) logger10.info(`[tracemoe] 正在为高置信度结果下载视频预览...`);
        const videoPreview = await ctx.http.get(result.url, { responseType: "arraybuffer" });
        figureMessage.children.push((0, import_koishi11.h)("message", { nickname: "视频预览", avatar: botUser.avatar }, import_koishi11.h.video(videoPreview, "video/mp4")));
      } catch (e) {
        logger10.warn(`[tracemoe] 高置信度视频预览下载失败: ${e.message}`);
      }
    }
    for (const enhancer of sortedEnhancers) {
      try {
        ...
        if (enhancedData) {
          if (config.debug.enabled) logger10.info(`[${enhancer.name}] 已成功获取图源信息。`);
          if (enhancedData.imageBuffer) {
            figureMessage.children.push((0, import_koishi11.h)("message", { nickname: "图源图片", avatar: botUser.avatar }, import_koishi11.h.image(enhancedData.imageBuffer, enhancedData.imageType)));
          }
          const enhancedDetailsNode = (0, import_koishi11.h)("message", { nickname: "图源信息", avatar: botUser.avatar }, enhancedData.details);
          figureMessage.children.push(enhancedDetailsNode);
          break;
        }
      } catch (e) {
        logger10.warn(`[${enhancer.name}] 图源处理时发生错误:`, e);
      }
    }
  }
  __name(buildHighConfidenceMessage, "buildHighConfidenceMessage");
  async function createResultContent(result) {
    const textFields = [
    ...
    const textNode = import_koishi11.h.text("\n" + textFields.join("\n"));
    try {
      ...
      return [import_koishi11.h.image(dataUri), textNode];
    } catch (e) {
      logger10.warn(`缩略图下载失败 ${result.thumbnail}:`, e.message);
      return [(0, import_koishi11.h)("p", "[!] 缩略图加载失败"), textNode];
    }
  }
  __name(createResultContent, "createResultContent");
  async function buildLowConfidenceNode(result, engineName, botUser) {
    const content = await createResultContent(result);
    return (0, import_koishi11.h)("message", {
  ...
  function getImageUrlAndName(session, text) {
    ...
    if (config.debug.enabled && url) {
      logger10.info(`[Debug] Parsed image URL from elements: ${url}`);
    }
