...
var Config = import_koishi.Schema.object({
  ...
  gelbooru: import_koishi.Schema.object({
    enabled: import_koishi.Schema.boolean().default(false).description("启用 Gelbooru 图源。"),
  ...
  danbooru: import_koishi.Schema.object({
    enabled: import_koishi.Schema.boolean().default(false).description("启用 Danbooru 图源。"),
    keyPairs: import_koishi.Schema.array(import_koishi.Schema.object({
  ...
  for (const name2 in enhancerRegistry) {
    ...
    if (generalConfig.enabled) {
      if (!entry.needsKeys || Array.isArray(entry.keys) && entry.keys.length > 0) {
        ...
        logger10.info(`[${name2}] ${entry.messageName}已启用但未配置任何${entry.keyName}，已禁用。`);
      }
    }
  }
