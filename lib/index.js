...
var PuppeteerManager = class {
  ...
  async getBrowserPath() {
    if (this.config.chromeExecutablePath) {
      if (this.config.debug.enabled) logger12.info(`[Stealth] 使用用户配置的全局浏览器路径: ${this.config.chromeExecutablePath}`);
...
*	返回的搜索结果可能存在 **NSFW** 内容，请设置分级筛选或在合理范围内使用。
`;
  ...
  ctx.command("sauce [...text:string]", "聚合搜图").alias("soutu", "搜图").option("all", "-a, --all 返回所有启用的引擎搜索结果").action(async ({ session, options }, text) => {
    ...
    if (!imgData.url) {
      ...
      try {
        ...
        imgData = getImageUrlAndName({ content: unescapedContent, quote: session.quote, elements: import_koishi16.h.parse(unescapedContent) }, unescapedContent);
